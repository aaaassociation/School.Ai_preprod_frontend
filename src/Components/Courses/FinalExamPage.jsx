...
const FinalExamPage = () => {
    ...
    useEffect(() => {
        if (prompt) {
            const fetchExamQuestions = async () => {
                setIsLoading(true);
                try {
                    const response = await fetch(`${process.env.REACT_APP_API_URL}/generate-final-exam`, {
                        method: 'POST',
                        headers: {
                    ...
                    console.log("Fetched Final Exam Questions:", data); // Debug output
                    setQuestions(data);
                    setIsLoading(false);
                } catch (error) {
                    console.error('There was an error fetching the final exam questions:', error);
                    setIsLoading(false);
                }
            };
    ...
    const handleSubmitExam = async () => {
        setIsLoading(true);
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/evaluate-final-exam`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            const data = await response.json();
            setResults(data);
            console.log("Final Exam Results:", data); // Debug output
            setIsLoading(false);
        } catch (error) {
        ...
        <>
            <div className={`${styles.sectionPadding} ${styles.bgCover} ${styles.bgNoRepeat} ${styles.bgCenter} ${styles.minHScreen} ${styles.flex} ${styles.itemsCenter} ${styles.justifyCenter}`} style={{ marginTop: "100px" }}>
                <div className={`${styles.container} ${styles.bgWhite} ${styles.shadowBox5} ${styles.rounded} ${styles.p8}`}>
                    <h1 className={`${styles.text3xl} ${styles.fontBold} ${styles.textCenter}`}>Final Exam for {prompt}</h1>
                    {isLoading ? (
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); handleSubmitExam(); }}>
                            ...
                            <button type="submit" className={`${styles.btn} ${styles.btnPrimary} ${styles.mt4}`}>Submit Final Exam</button>
                        </form>
                    ) : (
                        <p>Loading final exam questions...</p>
                    )}
                    {results && (
