...
const ViewHistoryCoursePage = () => {
  ...
  const handleVideoGenerate = async (url, text, isQuestion = false) => {
    ...
    try {
      ...
      const videoResponse = await fetch(`${process.env.REACT_APP_API_URL}/generate-avatar`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleExplainClick = () => {
    handleVoiceGenerate(`${process.env.REACT_APP_API_URL}/fetch-explanation`, prompt);
  };

  const handleAskQuestionClick = () => {
    handleVoiceGenerate(`${process.env.REACT_APP_API_URL}/ask-question`, questionText, true);
  };

  const handleExplainVideoClick = () => {
    handleVideoGenerate(`${process.env.REACT_APP_API_URL}/generate-explanation`, prompt);
  };

  const handleAskQuestionVideoClick = () => {
    handleVideoGenerate(`${process.env.REACT_APP_API_URL}/ask-question`, questionText, true);
  };

  const handleDigDeeper = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/dig-deeper`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleTakeExam = () => {
    window.location.href = "/schoolai/exam"; // Navigate to the exam page
  };

  const handleTakeFinalExam = () => {
    window.location.href = "/schoolai/final-exam"; // Navigate to the final exam page
  };
    ...
    <>
      <div className={`${styles.sectionPadding} ${styles.bgCover} ${styles.bgNoRepeat} ${styles.bgCenter} ${styles.minHScreen} ${styles.flex}`} style={{ marginTop: "100px" }}>
        <div className={styles.toc}>
          ...
          {/* Final Exam Option styled like a chapter */}
          <div>
            <h3 className={styles.chapterTitle} onClick={handleTakeFinalExam}>
              Final Exam
            </h3>
          </div>
        </div>
        <div className={`${styles.courseContentPage} ${styles.bgWhite} ${styles.shadowBox5} ${styles.rounded} ${styles.p8}`}>
          ...
          <div className={styles.courseContent}>
            ...
            <div className={styles.buttonGroup}>
              <button onClick={handleTakeExam} className={`${styles.btn} ${styles.btnPrimary} ${styles.mt4}`} style={{ marginRight: "15px" }}>
                Take Exam
              </button>
